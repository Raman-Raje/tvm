...
void IRBuilder::RegisterSType(const std::string& name, SType var_stype) {
  ...
  if (it != stype_name_tbl_.end()) {
    TVM_FFI_THROW(InternalError) << name << " already exists.";
    return;
  }
...
SType IRBuilder::QuerySType(const std::string& name) {
  ...
  TVM_FFI_THROW(InternalError) << "Value \"" << name << "\" does not yet exist.";
  return SType();
}
...
spv::ImageFormat IRBuilder::GetImageFormat(const PrimType& dtype, int channels) {
...
SType IRBuilder::GetStorageImageSType(const PrimType& dtype, int num_dimensions, uint32_t sampled) {
  ...
  if (spv_format == spv::ImageFormatUnknown) {
    TVM_FFI_THROW(InternalError) << "Unsupported image format for dtype: "
  ...
  if (num_dimensions == 1) {
    ...
    TVM_FFI_THROW(InternalError) << "Unsupported number of dimensions: " << num_dimensions;
  }
...
Value IRBuilder::StorageImageArgument(const std::string& name, const PrimType& dtype,
