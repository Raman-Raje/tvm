...
class IRBuilder {
 public:
  ...
  spv::ImageFormat GetImageFormat(const tvm::PrimType& dtype, int channels);

  Value StorageImageArgument(const std::string& name, const tvm::PrimType& dtype,
                             int num_dimensions,
                             uint32_t sampled, uint32_t descriptor_set, uint32_t binding);

  SType GetStorageImageSType(const tvm::PrimType& dtype, int num_dimensions, uint32_t sampled);
