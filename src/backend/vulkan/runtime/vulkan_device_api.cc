...
#include "../../../runtime/memory/pooled_allocator.h"
...
void VulkanDeviceAPI::CopyDataFromTo(DLTensor* from, DLTensor* to, TVMStreamHandle stream) {
  ...
  TVM_FFI_ICHECK(from->device.device_type == kDLVulkan || from->device.device_type == kDLCPU);
  TVM_FFI_ICHECK(to->device.device_type == kDLVulkan || to->device.device_type == kDLCPU);
      ...
      .def_packed("device_api.vulkan.alloc_nd",
                  [](ffi::PackedArgs args, ffi::Any* rv) {
                    ...
                    *rv = VulkanDeviceAPI::Global()->AllocDataSpace(
                        ...
                        ffi::Optional<ffi::String>(ffi::String("global.texture")));
                  })
 ...
 public:
  ...
  void* CreateView(const Buffer& buffer, ffi::Shape shape, DLDataType type_hint,
    ...
    return VulkanDeviceAPI::Global()->AllocDataSpaceView(
        buffer.device, buffer.data, shape, type_hint, ffi::Optional<ffi::String>(ffi::String(mem_scope)));
  }
